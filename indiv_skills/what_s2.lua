...
skill.set_ability = function(self, skill, initial, delay_sprites)
    ...
    if valid_cards[1] then 
        local _card = pseudorandom_element(valid_cards, pseudoseed('what_s2'..G.GAME.round_resets.ante))
            ...
            func = function()
                for _, v in ipairs(G.hand.cards) do
                    if v.base.id == skill.ability.id and not v.ability.eternal then
                        ...
                        v:start_dissolve() 
                        if first then play_sound("lobc_what_s2", math.random(90, 110)/100, 0.5) end
                        first = nil
                        delay(0.23)
                    end
                end
